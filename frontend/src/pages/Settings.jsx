...
const Settings = () => {
  // ✅ Zustand user store (exact copy from Profile)
  const { user, loading, loadUser } = useUserStore();
  
  // ✅ Load user on mount (exact copy from Profile)
  useEffect(() => {
    loadUser();
  }, [loadUser]);
  ...
  // ✅ Loading state (exact copy from Profile)
  if (loading) {
    return (
  ...
  const handleSaveGroqKey = async () => {
    ...
    setIsSaving(true)
    try {
      // Save to HashiCorp Vault via your backend
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 py-12 px-4 flex">
      <Sidebar /> {/* ✅ Now shows user name via Zustand */}
      
      <div className="max-w-2xl mx-auto"> {/* ✅ No flex-1 - perfect layout */}
        <div className="bg-white rounded-3xl shadow-2xl p-8 mb-8 border border-gray-200">
          <div className="mb-12">
            ...
            <p className="text-gray-600 mt-2 text-lg">Manage your API integrations</p>
          </div>
            ...
            {isConnected ? (
              <div className="bg-gradient-to-r from-emerald-50 to-green-50 border border-emerald-200 rounded-3xl p-8 shadow-xl">
                <div className="flex items-center justify-center text-center space-y-4">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold text-emerald-800 mb-2">Connected ✅</h3>
                    <p className="text-emerald-700 font-medium">Securely stored in HashiCorp Vault</p>
                  </div>
                  <button 
                ...
                <div className="space-y-4">
                  ...
                  >
                    {isSaving ? (
                      ...
                      '🔒 Save Securely to HashiCorp Vault'
                    )}
                  </button>
                </div>
